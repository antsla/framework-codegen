...
func TestOtherApi(t *testing.T) {
	...
	cases := []Case{
		Case{
			Path:   ApiUserCreate,
			Method: http.MethodPost,
			Query:  "username=I3apBap&level=1&class=barbarian&account_name=Vyacheslav",
			Status: http.StatusBadRequest,
			Auth:   true,
			Result: CR{
		...
		Case{
			Path:   ApiUserCreate,
			Method: http.MethodPost,
			Query:  "username=I3apBap&level=1&class=warrior&account_name=Vyacheslav",
			Status: http.StatusOK,
			Auth:   true,
			Result: CR{
				"error": "",
